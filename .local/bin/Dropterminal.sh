...
animate_slide_down() {
  local addr="$1"
  local target_x=$(hyprctl clients -j | jq -r --arg ADDR "$addr" '.[] | select(.address==$ADDR) | .at[0]')
  local target_y="$3"
  local width="$4"
  local height="$5"
...
# move window completely offscreen first
offscreen_y=$((curr_y - curr_height - 200))
hyprctl dispatch movewindowpixel "exact $curr_x $offscreen_y,address:$TERMINAL_ADDR"
