...
hl.config({
    input = {
        ...
        kb_options      = "compose:caps,shift:both_capslock_cancel", -- CapsLock = compose; 2×Shift = Caps Lock (self-clears)
        kb_rules        = "",            -- XKB keymap rules.
        kb_file         = "",            -- Path to a custom .xkb file (overrides the above).
        numlock_by_default   = true,     -- Engage numlock on startup.
        resolve_binds_by_sym = false,    -- How keybinds behave with multiple layouts active.
        repeat_rate      = 40,           -- Key repeat rate in repeats/sec.
        ...
        touchpad = {
            ...
            scroll_factor           = 0.4,   -- Touchpad scroll speed multiplier.
            middle_button_emulation = false, -- LMB + RMB simultaneously = middle click.
            tap_button_map          = "",    -- Tap button mapping: "" | "lrm" | "lmr".
            clickfinger_behavior    = true,  -- 1/2/3-finger click = LMB/RMB/MMB.
...
-- Wake the display from DPMS sleep on any key/mouse input.
hl.config({
    misc = {
...
hl.config({
    cursor = {
        ...
        no_hardware_cursors          = 0,      -- HW cursor mode: 0=force hw 1=force sw 2=auto.
        no_break_fs_vrr              = 1,      -- Skip frame scheduling on cursor move in FS+VRR: 0=off 1=on 2=auto.
