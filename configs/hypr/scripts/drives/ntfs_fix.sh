...
check_deps() {
    ...
    for cmd in "${deps[@]}"; do
        if ! command -v "$cmd" &>/dev/null; then
            if [[ "$cmd" == "ntfsfix" ]]; then
                log_err "Missing dependency: 'ntfsfix'. Install with: sudo pacman -S ntfsprogs"
            fi
...
repair_volume() {
    ...
    local confirm
    read -rp "${C_BOLD}Proceed with ntfsfix (clears dirty bit + bad-sector list) on ${target}? [y/N] ${C_RESET}" confirm
    if [[ ! "$confirm" =~ ^[Yy]$ ]]; then
