...
def manipulate_core_state(cpu_ids: list[int], state: str) -> bool:
    if not cpu_ids:
        return True
    if state == "0":
        running_on = os.getcpu()
        if any(c == running_on for c in cpu_ids):
            console.print("[bold red]Safety:[/bold red] refusing to offline the core this process is running on.")
            return False
        online_now = [c for c in cpu_ids if get_core_status(c)]
        if online_now:
            total_online = sum(1 for c in range(os.cpu_count() or 1) if get_core_status(c))
            if total_online - len(online_now) < max(1, total_online // 2):
                console.print("[bold red]Safety:[/bold red] refusing to offline more than half the online cores.")
                return False
