...
# Safety lockdown: dry-run by default; require --apply to make changes
DRY_RUN=true
APPLY=false
readonly ALLOWED_DIRS=("$HOME/Pictures" "$HOME/Downloads")
...
# --- Allowlist guard ---
# Scripts must never touch arbitrary paths. Apply-mode is only allowed when
# the working directory is inside an allowlisted location.
in_allowlist() {
    local current
    current="$(realpath -m "$PWD" 2>/dev/null || printf '%s' "$PWD")"
    local allowed
    for allowed in "${ALLOWED_DIRS[@]}"; do
        local base
        base="$(realpath -m "$allowed" 2>/dev/null || printf '%s' "$allowed")"
        [[ "$current" == "$base" || "$current" == "$base/"* ]] && return 0
    done
    return 1
}
...
convert_and_archive() {
    ...
    if [[ "$DRY_RUN" == true ]]; then
        log_info "[DRY] Would convert: $src -> $dst"
        return 0
    fi
...
Usage: ${0##*/} [--apply|--dry-run]
  Converts all PNGs in the current directory to JPG.
  SAFETY: dry-run (preview only) is the default. Pass --apply to actually
  convert, and only when run from an allowlisted directory.
EOF
                exit 0 ;;
    ...
    if [[ "$DRY_RUN" == true ]]; then
        log_info "DRY RUN enabled - no files will be converted or moved."
    else
        if ! in_allowlist; then
            log_err "Refusing to apply from non-allowlisted directory: $PWD"
            log_err "Allowed: ${ALLOWED_DIRS[*]}"
            exit 1
        fi
    fi
    ...
    if [[ "$DRY_RUN" == true ]]; then
        printf '\n%sRun with --apply to actually convert (allowlisted dirs only).%s\n' "$BOLD" "$RESET"
    fi
}
