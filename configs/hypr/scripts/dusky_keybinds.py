...
#              - Checks for 'rich' and asks the user to install it manually if missing
...
def ensure_dependencies() -> None:
    try:
        ...
        print("\033[0;36m[*] Install it manually, then re-run this script.\033[0m")
...
# Omarchy default keybinds are split across several files in this dir.
SOURCE_DIR = Path('/usr/share/omarchy/default/hypr/bindings')

def read_source_binds() -> str:
    """Concatenate every *.lua in the Omarchy default bindings dir."""
    parts = []
    if SOURCE_DIR.is_dir():
        for p in sorted(SOURCE_DIR.glob('*.lua')):
            try:
                ...
                pass
    return '\n'.join(parts)
CUSTOM_LUA = XDG_CONFIG_HOME / 'hypr/bindings.lua'

VIEW_ONLY = False
EMPTY_TEMPLATE = 'o.bind("SUPER + ", "", "")'
...
class Bind:
    ...
    is_orphan:  bool = False  # unbind with no active o.bind (key silently dead)


# Keys the user intentionally left as unbind-only (no replacement) — NOT bugs.
# Do not flag these as orphan / dead in the UI or the health warning.
INTENTIONAL_UNBINDS = {
    # Tuples use normalize_key() format: (mods, key, submap) with '+' joined, lowercase.
    ('alt+super', 'space', ''),         # Apps menu removed on purpose
    ('shift+super', 's', ''),           # Sidepad S disabled on purpose
    ('ctrl', 'tab', ''),                # window switcher replaced by gloview
    ('super+ctrl', 'space', ''),        # intentional disable
    ('super+shift', 'tab', ''),         # gloview desktop
    ('super+shift+ctrl', 'space', ''),  # intentional disable
}
...
def backup_file(path: Path) -> None:
    """Copy path -> path.timestamp.bak before an edit."""
    try:
        if path.exists():
            ts = datetime.now().strftime('%Y%m%d-%H%M%S')
            path.replace(path.with_suffix(path.suffix + f'.{ts}.bak'))
    except Exception:
        pass
...
def get_editor_choice(source_binds: list[Bind] | None = None,
                       custom_binds: list[Bind] | None = None) -> str:
    """Dynamically probes for $EDITOR before offering fallbacks.

    Runs a one-time orphan-unbind health audit (if both bind lists are
    supplied) so the user is warned about silently-dead keybinds before
    they start editing.
    """
    ...
    # ── Keybind health audit (omit if you dislike the warning) ──
    # NOTE: previously referenced a bare `custom_binds` that was never in
    # scope here, so this silently raised NameError and the warning never
    # fired. Callers now pass the live bind lists.
    if custom_binds is not None:
        try:
            _orphans = audit_orphan_unbinds(custom_binds)
            if _orphans:
                print_orphan_warning(_orphans)
        except Exception:
            pass
...
def parse_lua_file_content(text: str, origin: str) -> list[Bind]:
    ...
    for m in re.finditer(r'(o|hl)\.(bind|unbind)\s*\(', text):
        start = m.start()
        if not mask[start]: continue

        fn = m.group(1)
        is_unbind = (m.group(2) == 'unbind')
        paren_pos = text.find('(', start)
        end = find_balanced_end(text, paren_pos, mask)
        ...
        if is_unbind:
            ...
            else:  # o.bind(key, desc, cmd)  desc may be string, nil, or table
                options = ''
                first = args[1].strip()
                if first.startswith('"') or first.startswith("'"):
                    description = strip_quotes(first)
                    dispatcher = args[2].strip() if len(args) > 2 else ''
                else:
                    description = ''
                    dispatcher = first
...
def format_display(b: Bind) -> str:
    ...
    if b.is_orphan:
        return f'{GREY}[DEAD]{RESET}  {submap_pfx}{GREY}{ui_key}{RESET} {GREY}│{RESET} {DIM}no o.bind - key silently dead{RESET}'
    if b.is_unbind:
...
def generate_bind_rows(source_binds: list[Bind], custom_binds: list[Bind]) -> tuple[list[str], list[Bind]]:
    ...
    orphan_keys = custom_ovr - custom_active  # unbinds with no active bind
    ...
    for b in sorted(custom_binds, key=lambda x: f'{x.submap}|{x.norm_mods}|{x.norm_key}'):
        ...
        if b.is_unbind and (b.norm_mods, b.norm_key, b.submap) in orphan_keys \
           and (b.norm_mods, b.norm_key, b.submap) not in INTENTIONAL_UNBINDS:
            b.is_orphan = True
        displayed.append(b)
...
def edit_loop(bind: Optional[Bind], source_binds: list[Bind], custom_binds: list[Bind]) -> bool:
    ...
    editor_choice = get_editor_choice(source_binds, custom_binds)
...
def delete_flow(bind: Bind) -> bool:
    ...
    if bind.origin == 'SRC':
        ...
        # NOTE: backup_file() is already called once above; the second call
        # that used to be here was a redundant copy-paste bug (the file is
        # already renamed to .bak on the first call, so the second was a no-op).
    else:
...
def audit_orphan_unbinds(custom_binds: list[Bind]) -> list[Bind]:
    """Return custom unbind directives that have NO matching active o.bind.
    These 'orphan unbinds' silently kill a key (source hidden, no replacement)."""
    active = {(b.norm_mods, b.norm_key, b.submap) for b in custom_binds if not b.is_unbind}
    orphans = [b for b in custom_binds if b.is_unbind and (b.norm_mods, b.norm_key, b.submap) not in active
               and (b.norm_mods, b.norm_key, b.submap) not in INTENTIONAL_UNBINDS]
    return orphans

def print_orphan_warning(orphans: list[Bind]) -> None:
    if not orphans:
        return
    lines = "\n".join(f"  • {b.norm_mods} + {b.norm_key}" + (f"  [submap:{b.submap}]" if b.submap else "") for b in orphans)
    console.print(Panel(
        f"[bold red]Found {len(orphans)} orphan unbind(s) — keys with hl.unbind but NO o.bind.[/bold red]\n"
        f"[yellow]These keys are SILENTLY DEAD (source hidden, no replacement):[/yellow]\n{lines}\n\n"
        f"[cyan]Fix: open each via 'Create/Edit', or restore the o.bind line in {CUSTOM_LUA}.[/cyan]",
        title="[bold red]KEYBIND HEALTH WARNING[/bold red]", border_style="red", expand=False))
