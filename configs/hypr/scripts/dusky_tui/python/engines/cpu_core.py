...
def _current_cpu_id() -> int:
    try:
        with open("/proc/self/stat") as f:
            data = f.read()
        after_comm = data.rsplit(")", 1)[1].split()
        # After the comm field, fields are 3..N; processor is field 39 => index 36.
        return int(after_comm[36])
    except (OSError, ValueError, IndexError):
        return -1

def _online_core_ids() -> list[int]:
    online = []
    for node in Path("/sys/devices/system/cpu").glob("cpu[0-9]*"):
        if not node.is_dir():
            continue
        online_file = node / "online"
        if online_file.exists() and safe_read(online_file) == "1":
            online.append(int(node.name[3:]))
    return online
...
def set_core_status(cpu_id: int, enable: bool) -> tuple[bool, str]:
    ...
    if not enable:
        # Safety guard: never offline the core this process is running on.
        try:
            if _current_cpu_id() == cpu_id:
                return False, "Safety guard: refusing to offline the currently running core"
        except Exception:
            pass
        # Safety guard: never leave fewer than half the online cores active.
        online_now = _online_core_ids()
        total_online = len(online_now)
        if cpu_id in online_now:
            total_online -= 1
        if total_online < max(1, len(online_now) // 2):
            return False, "Safety guard: refusing to offline more than half of the online cores"
