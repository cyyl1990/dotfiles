...
class PkgThrottleEngine(BaseEngine):
    ...
    def write_value(self, target_key: str, target_scope: str, new_value: str, item_type: str = "string") -> tuple[bool, str, str]:
        ...
        # Safety guard: PL4 (peak) must never be set below the current PL2 (short-term) limit.
        # Setting PL4 too low can trigger a failsafe hardware lock (minimum clock throttle).
        if target_key == "pl4":
            pl2_raw = safe_read_int(self.domain / "constraint_1_power_limit_uw")
            if pl2_raw is not None and val < pl2_raw:
                pl2_disp = f"{pl2_raw // 1_000_000} W"
                return False, f"Safety guard: PL4 cannot be lower than current PL2 ({pl2_disp})", ""
