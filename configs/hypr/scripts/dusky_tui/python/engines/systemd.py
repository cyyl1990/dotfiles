...
# System services that must never be disabled through this manager.
# Disabling any of these can break boot, networking, or the display session.
CRITICAL_SYSTEM_UNITS = {
...
class SystemdEngine(BaseEngine):
    ...
    def write_value(self, target_key: str, target_scope: str, new_value: str, item_type: str = "bool") -> tuple[bool, str, str]:
        ...
        # Safety guard: never disable critical system units at system scope.
        if action == "disable" and target_scope != "user" and target_key in CRITICAL_SYSTEM_UNITS:
            return False, f"Safety guard: {target_key} is a critical system service and cannot be disabled", ""
    ...
    def write_batch(self, changes: list[tuple[str, str, str, str]]) -> tuple[bool, str, str]:
        ...
        for key, scope, val_str, _ in changes:
            if scope == "user":
                ...
                # Safety guard: critical system units are filtered out of system disable batches.
                elif key not in CRITICAL_SYSTEM_UNITS:
                    sys_disable.append(key)
