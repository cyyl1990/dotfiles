...
# =============================================================================
# OMARCHY THPM THEME SOURCE
# On this machine the active theme lives in Omarchy's THPM colors.toml
# (matugen output is no longer used). Palette roles follow dusky_keybinds.py.
# =============================================================================
def resolve_omarchy_colors_file() -> Path | None:
    """Resolve the active Omarchy theme's colors.toml (same logic thpm uses)."""
    candidates = [
    ...
    for c in candidates:
        if c.is_file():
            return c
    if env := os.environ.get("THPM_CURRENT_THEME_DIR"):
        c = Path(env) / "colors.toml"
        if c.is_file():
            return c
    try:
        newest = max(
...
def load_omarchy_palette(file_path: Path | None = None) -> dict[str, str] | None:
    """Load Omarchy THPM colors.toml mapped to the Dusky TUI theme keys.

    Semantic roles follow dusky_keybinds.py:
    accent=cyan (color6), info=blue (color4), success=green (color2),
    warning=yellow (color3), error=red (color1), secondary=purple (color5),
    muted=grey (color8).
    """
    if tomllib is None:
        return None
    if file_path is None:
        file_path = resolve_omarchy_colors_file()
    if file_path is None or not file_path.is_file():
        return None

    try:
        colors = tomllib.loads(file_path.read_text(encoding="utf-8"))
    except Exception:
        return None

    def g(key: str, default: str) -> str:
        v = colors.get(key, default)
        return v if isinstance(v, str) and v.startswith("#") else f"#{v}"

    bg = g("background", "#111318")
    fg = g("foreground", "#e1e2e9")
    accent = g("color6", g("accent", "#26c6da"))
    return {
        "bg": bg,
        "fg": fg,
        "accent": accent,
        "info": g("color4", accent),
        "error": g("color1", "#f87171"),
        "warning": g("color3", "#fde047"),
        "success": g("color2", "#4ade80"),
        "secondary": g("color5", "#c792ea"),
        "muted": g("color8", "#43474e"),
    }
...
class DuskyTUI(App):
    CSS = """
/* Pacsea-style transparent backdrop: let the terminal background show
   through. Textual's default `App { background: $background; }` and the
   old Screen rule both painted a solid #090E13 layer over it. */
...
"""
    ...
    def __init__(
        ...
        **kwargs
        ...
        # Theme colors — Dusky palette (semantics match dusky_keybinds.py:
        # accent=cyan, info=blue, success=green, warning=yellow, error=red,
        # secondary=purple, muted=grey). Overridden live by Omarchy THPM colors.toml.
        self.theme_colors = {
            "bg": "#111318",
            "fg": "#e1e2e9",
            "accent": "#26c6da",
        ...
        # On this machine the theme source is Omarchy's THPM hook colors.toml
        # (matugen output is no longer used).
        omarchy_colors = resolve_omarchy_colors_file()
        if omarchy_colors:
            self.theme_path = omarchy_colors

        self.last_theme_mtime: float = 0.0
        if self.theme_path:
            loaded_theme = load_omarchy_palette(self.theme_path)
            if loaded_theme:
    ...
    async def watch_theme_file(self) -> None:
        ...
        try:
            ...
            if current_mtime > self.last_theme_mtime:
                new_theme = await asyncio.to_thread(load_omarchy_palette, self.theme_path)
    ...
    def apply_theme_to_engine(self) -> None:
        ...
        theme_name = "dusky_omarchy_A" if self._theme_toggle else "dusky_omarchy_B"

        bg = self.theme_colors.get("bg", "#111318")
        fg = self.theme_colors.get("fg", "#e1e2e9")
        accent = self.theme_colors.get("accent", "#26c6da")
        muted = self.theme_colors.get("muted", "#43474e")
        err = self.theme_colors.get("error", "#f87171")
        warn = self.theme_colors.get("warning", "#fde047")
        succ = self.theme_colors.get("success", "#4ade80")
