...
# -----------------------------------------------------------------------------
# FZF MENU MODE (no rofi required)
# Presents a terminal UI to pick orientation + preset, then applies it.
# -----------------------------------------------------------------------------
fzf_select() {
    if ! command -v fzf &>/dev/null; then
        echo "fzf not found. Install fzf or use rofi mode." >&2
        return 1
    fi

    get_current_state

    # Top-level: 4 clear choices (Default/Disabled are presets, Vertical/Horizontal open a preset sub-menu)
    local top
    top=$(printf 'Default (Dusky)\nDisabled\nVertical\nHorizontal\n' | fzf \
        --prompt="Animation> " --height=~40% --border --ansi \
        --header="Current: ${current_anim##*/} (${current_orient})")
    [[ -z "$top" ]] && return 1

    case "$top" in
        "Default (Dusky)")
            # Raw dusky.lua, no orientation awk transform (matches 006_animation_default.sh)
            apply_animation_raw "$ANIM_DIR/dusky.lua"
            return $?
            ;;
        "Disabled")
            ...
            ;;
        "Vertical"|"Horizontal")
            local orient="${top,,}"   # lowercase
            shopt -s nullglob
            local files=("$ANIM_DIR"/*.lua)
            shopt -u nullglob
            local choices=()
            for f in "${files[@]}"; do
                [[ "${f##*/}" == "disable.lua" ]] && continue
                # Hide dusky from the sub-menus: it already has its own "Default" entry
                [[ "${f##*/}" == "dusky.lua" ]] && continue
                choices+=("${f##*/}")
            done
            if [[ ${#choices[@]} -eq 0 ]]; then
                echo "No animation presets found in $ANIM_DIR" >&2
                return 1
            fi
            local pick
            pick=$(printf '%s\n' "${choices[@]}" | fzf \
                --prompt="Animation ($orient)> " --height=~60% --border --ansi \
                --header="Current: ${current_anim##*/}")
            [[ -z "$pick" ]] && return 1
            apply_animation "$orient" "$ANIM_DIR/$pick"
            return $?
            ;;
    esac
}

# Apply a lua file as-is (no orientation awk transform). Used by "Default (Dusky)".
apply_animation_raw() {
    local src_file="$1"
    if [[ ! -f "$src_file" ]]; then
    ...
    if cp -- "$src_file" "$tmp_file"; then
        if mv -f -- "$tmp_file" "$DEST_FILE"; then
            ...
            printf '%s|%s\n' "default" "$src_file" > "$STATE_FILE"
            reload_hyprland
            notify_user "Animation Applied" "${src_file##*/} (Default)"
            return 0
        fi
    fi
