...
declare DRY_RUN=true USE_PARALLEL="auto" INTERACTIVE=false APPLY=false
...
run_interactive() {
    ...
    # Dry run? Default to preview (safe)
    printf '\n' >&2
    if confirm "Preview first (dry-run)?" "y"; then
        DRY_RUN=true
    else
        DRY_RUN=false
    fi
...
${C_BOLD}OPTIONS${C_RESET}
    ...
    --apply               Actually move/rename (allowlisted dirs only)
    -h, --help            Show help
EOF
}
...
main() {
    ...
    # Safety: applying is only allowed from allowlisted dirs
    if [[ "$DRY_RUN" == false ]] && ! in_allowlist; then
        die "Refusing to apply outside allowlisted dirs. Allowed: ${ALLOWED_DIRS[*]}"
    fi
    if [[ "$DRY_RUN" == true ]]; then
        log_warn "DRY RUN - no files will be modified. Pass --apply to apply."
    fi
