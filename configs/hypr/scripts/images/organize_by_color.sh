...
in_allowlist() {
    local target base
    target="$(realpath -m "$SRC_DIR" 2>/dev/null || printf '%s' "$SRC_DIR")"
...
${BOLD}Options:${RESET}
  -n, --dry-run   Show what would be done without renaming files (default)
  --apply         Actually rename files (allowlisted dirs only)
...
${BOLD}Examples:${RESET}
  $(basename "$0")                          # Preview (dry-run) in current directory
  $(basename "$0") --apply                  # Actually rename
  $(basename "$0") -u --apply               # Undo: strip color prefixes
EOF
}
while [[ $# -gt 0 ]]; do
    case "$1" in
...
if [[ "$DRY_RUN" == false ]] && ! in_allowlist; then
    err "Refusing to apply outside allowlisted dirs. Allowed: ${ALLOWED_DIRS[*]}"
    exit 1
fi
if [[ "$DRY_RUN" == true ]]; then
    info "DRY RUN - preview only. Pass --apply to actually rename."
fi
...
if [[ "$DRY_RUN" == true ]]; then
    ...
    [[ $SKIPPED -gt 0 ]] && warn "$SKIPPED file(s) already tagged correctly" || true
    [[ $FAILED -gt 0 ]]  && warn "$FAILED file(s) could not be classified" || true
fi