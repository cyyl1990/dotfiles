...
def set_core_state(cpu_id: int, state: str) -> None:
    ...
    if state == "0" and os.getcpu()[0] == cpu_id:
        print(f"Skipping core {cpu_id}: refusing to offline the core this process is running on.", file=sys.stderr)
        return
...
def main() -> None:
    ...
    if args.offline:
        ...
        for cpu in filter(None, (p.strip() for p in args.offline.split(','))):
            try:
                offline_ids.append(int(cpu))
            except ValueError:
                sys.exit(f"Invalid core ID: {cpu}")

        online_ids = []
        for c in range(os.cpu_count() or 1):
            p = Path(f"/sys/devices/system/cpu/cpu{c}/online")
            if p.exists() and p.read_text().strip() == "1":
                online_ids.append(c)
        if offline_ids and online_ids:
            remaining = len(online_ids) - len(offline_ids)
            if remaining < max(1, len(online_ids) // 2):
                sys.exit(f"Safety guard: refusing to offline {len(offline_ids)} cores "
                         f"(would leave {remaining} online). Keep at least half the cores online.")
        for cpu_id in offline_ids:
            try:
                set_core_state(cpu_id, "0")
            except ValueError:
                sys.exit(f"Invalid core ID: {cpu_id}")
