...
# Forward-only. No legacy shims. No auto-install — reports missing deps and exits.
#
# What’s new in v8 vs v7:
#  • Pre-launch bootstrap: detects missing python-textual, python-rich, wl-clipboard,
#    hyprland, reports missing deps and asks user to install manually (no auto-install)
...
def _bootstrap_arch_deps() -> None:
    ...
    if shutil.which("pacman") is not None:
        sys.exit(f"Install missing dependencies manually, then re-run:\n  sudo pacman -S {' '.join(missing_pacman)}")
    sys.exit(f"Install missing dependencies manually, then re-run:\n  {' '.join(missing_pacman)}")

# Bootstrap is run inside main() (guarded by __main__) so importing this module
# from the plain-CLI variant does NOT trigger pacman/sudo.
...
# ---------------------------------------------------------------------------
RuleText = str
TARGET_FILE = Path.home() / ".config" / "hypr" / "rules.lua"
...
# ---------------------------------------------------------------------------
# 6.5 Theme — pull the ACTIVE Omarchy/thpm palette so the TUI follows the
#      system theme live (same source every theme-set.d hook reads:
#      ~/.local/state/omarchy/current/theme/colors.toml). No hardcoded colours.
# ---------------------------------------------------------------------------
def _lighten(hex_color: str, amount: float) -> str:
    """Lighten a #RRGGBB by `amount` (0..1) toward white. Falls back to input."""
    try:
        h = hex_color.lstrip("#")
        if len(h) == 3:
...
def load_omarchy_palette() -> dict[str, str]:
    """Resolve the active theme's colours.toml (source of truth for Omarchy)."""
    candidates: list[Path] = []
    for env in ("THPM_COLORS_FILE", "THPM_COLORS_FILE_RESOLVED"):
        ...
        if c and c.is_file():
            try:
                for line in c.read_text(encoding="utf-8").splitlines():
                    # Strip a trailing comment only if it is preceded by whitespace
                    # (so colour values like "#e75a50" are preserved).
                    stripped = re.sub(r"\s#.*$", "", line).strip()
                    m_ = re.match(r'^([A-Za-z0-9_]+)\s*=\s*["\']?([^"\']*)["\']?\s*$', stripped)
                    if m_:
                        data[m_.group(1)] = m_.group(2)
            except Exception:
                pass
            if data:
                break
    if not data:  # last resort: newest theme dir
        try:
            newest = max(
                (Path.home() / ".config" / "omarchy" / "themes").glob("*/colors.toml"),
    ...
    bg = g("background", "#1b1b1b")
    fg = g("foreground", "#efebdc")
    # Semantic roles follow dusky_keybinds.py: primary=cyan, info=blue,
    # secondary=purple, success=green, warning=yellow, error=red, muted=grey.
    accent = g("color6", g("accent", "#8ea4a2"))
    info = g("color4", accent)
    sec = g("color5", g("color13", "#c792ea"))
    succ = g("color2", g("color10", "#4ade80"))
    warn = g("color3", g("color11", "#fde047"))
    err = g("color1", g("color9", "#f87171"))
    muted = g("color8", g("color0", "#43474e"))
    sel_bg = g("selection_background", accent)
    sel_fg = g("selection_foreground", bg)
    return {
        "bg": bg,
        "fg": fg,
        "panel": _lighten(bg, 0.06),
        "border": _lighten(bg, 0.13),
        "accent": accent,
        "info": info,
        "secondary": sec,
        "success": succ,
        ...
        "sel_bg": sel_bg,
        "sel_fg": sel_fg,
        "bright": g("color15", g("foreground", "#efebdc")),
        "dim": _lighten(bg, 0.28),
        "dim2": g("color7", g("color15", "#aaaaaa")),
    }


def build_dusky_css(p: dict[str, str] | None = None) -> str:
    p = p or load_omarchy_palette()
    bg, fg, panel, border = p["bg"], p["fg"], p["panel"], p["border"]
    accent, sel_bg, sel_fg = p["accent"], p["sel_bg"], p["sel_fg"]
    bright, dim, dim2 = p["bright"], p["dim"], p["dim2"]
    info, sec, succ, warn, err = p["info"], p["secondary"], p["success"], p["warning"], p["error"]

    # Pacsea-style: fully transparent — terminal background shows through,
    # so the TUI blends seamlessly with whatever the system theme sets.
    TR = "transparent"

    # Derived tones for better visual hierarchy
    border_hi  = _lighten(bg, 0.20)   # slightly brighter border for focused sections
    hover_bg   = _lighten(bg, 0.09)   # subtle hover, between panel and border
    status_col = dim2                  # status bar uses mid-tone, not full dim
    badge_fg   = sel_fg               # badge text = selection foreground (high contrast)
    ...
    $footer-background:              {TR};
    $footer-foreground:              {fg};
    $footer-description-background:  {TR};
    $footer-description-foreground:  {dim2};
    $footer-key-background:          {TR};
    $footer-key-foreground:          {accent};
    $block-hover-background:         {hover_bg};

    /* Whole app must be transparent too — Textual's default `App` rule paints
       a solid $background, which would otherwise mask the terminal backdrop. */
    App {{
        background: {TR};
    }}
    ...
    /* ── Header ─────────────────────────────────────────────────────── */
    #header {{
        dock: top; height: 3; background: {TR};
        border-bottom: tall {border}; layout: horizontal; padding: 0 2;
    }}
    #title  {{ width: auto; color: {bright}; text-style: bold; padding-top: 1; }}
    #ver    {{ width: auto; color: {dim};    padding: 1 0 0 1; }}
    #preset-pill {{
        width: 1fr; content-align: right middle;
        color: {accent}; text-align: right; padding-top: 1; text-style: bold;
    }}

    /* ── Layout ──────────────────────────────────────────────────────── */
    #main    {{ layout: horizontal; height: 1fr; }}
    #sidebar {{
        width: 46; min-width: 32; max-width: 58;
        background: {TR}; border-right: solid {border}; layout: vertical;
    }}
    #sidebar-head {{
        height: 5; background: {TR};
        border-bottom: solid {border}; padding: 1 1 0 1; layout: vertical;
    }}
    #filter {{ margin-top: 1; }}
    #right  {{ width: 1fr; layout: vertical; background: {TR}; }}

    /* ── Window list ─────────────────────────────────────────────────── */
    #window-list {{
        height: 1fr; background: {TR};
        scrollbar-gutter: stable; scrollbar-size: 1 1;
    }}
    #window-list ScrollBar {{ color: {accent}; background: {TR}; }}
    ListItem          {{ background: {TR};     color: {fg};     }}
    ListItem.-selected {{ background: {sel_bg}; color: {sel_fg}; text-style: bold; }}
    ListItem:hover    {{ background: {hover_bg}; }}

    /* ── Preset bar ──────────────────────────────────────────────────── */
    #preset-bar {{
        height: 4; background: {TR}; layout: horizontal;
        padding: 1 1 0 1; border-bottom: solid {border};
    }}
    #preset-select {{ width: 38; }}
    #preset-desc   {{ width: 1fr; color: {dim2}; padding: 1 1 0 2; }}

    /* ── Preview pane ────────────────────────────────────────────────── */
    #preview-wrap {{
        height: 1fr; margin: 1 1 0 1;
        border: tall {border}; background: {TR};
        border-title-color: {bright}; border-title-style: bold;
        border-subtitle-color: {dim};
    }}
    #preview-wrap:focus-within {{ border: tall {accent}; border-title-color: {accent}; }}
    #preview {{
        background: {TR}; color: {fg};
        scrollbar-color: {accent}; scrollbar-background: {TR};
    }}

    /* ── Action buttons ──────────────────────────────────────────────── */
    #actions {{
        height: 3; layout: horizontal; background: {TR};
        padding: 0 1; border-top: solid {border};
    }}
    .btn         {{ min-width: 13; margin-right: 1; background: {panel};  color: {fg};      border: none; }}
    .btn:hover   {{ background: {hover_bg}; color: {bright}; }}
    .btn:focus   {{ background: {border};   color: {bright}; border: tall {accent}; }}
    .primary     {{ background: {accent};   color: {badge_fg}; text-style: bold; }}
    .primary:hover {{ background: {bright}; color: {bg}; }}

    /* ── Status bar ──────────────────────────────────────────────────── */
    #status   {{
        dock: bottom; height: 2; background: {TR};
        color: {status_col}; layout: horizontal; padding: 0 2;
    }}
    #status-l {{ width: 1fr; color: {dim2}; }}
    #status-r {{ width: auto; color: {dim}; }}

    /* ── Form widgets ────────────────────────────────────────────────── */
    Input        {{ background: {panel}; border: tall {border}; color: {fg}; }}
    Input.-valid {{ border: tall {border}; }}
    Input:focus  {{ border: tall {accent}; background: {hover_bg}; }}
    Input.-invalid:focus {{ border: tall {err}; }}

    Select        {{ background: {panel}; color: {fg}; border: tall {border}; }}
    Select:focus  {{ border: tall {accent}; background: {hover_bg}; }}
    SelectCurrent {{ color: {fg}; background: {TR}; }}
    SelectOverlay {{ background: {panel}; border: tall {border}; color: {fg}; }}
    SelectOverlay > .option-list--option-highlighted {{
        background: {sel_bg}; color: {sel_fg};
    }}

    /* ── Footer / keybinding bar ─────────────────────────────────────── */
    Footer {{ background: {TR}; color: {fg}; dock: bottom; height: 1; }}
    FooterKey {{ background: {TR}; }}
    FooterKey .footer-key--key         {{ color: {accent}; background: {TR}; text-style: bold; }}
    FooterKey .footer-key--description {{ color: {dim2};   background: {TR}; }}
    FooterKey:hover .footer-key--key   {{ color: {bright}; background: {panel}; }}
    FooterLabel {{ color: {dim}; background: {TR}; }}
    """
...
class DuskyApp(App[None]):
    ...
    def __init__(self, rules: list[GeneratedRule]) -> None:
        ...
        # Monotonic counter so every re-population of the ListView uses a
        # UNIQUE child id. ListView.clear() is async (AwaitRemove) — old
        # ListItems are still unmounting when we append again, so re-using a
        # fixed id like "w-0" raises DuplicateIds and crashes the TUI.
        self._populate_seq = 0
        ...
        with Horizontal(id="main"):
            ...
            with Vertical(id="right"):
                ...
                with Vertical(id="preview-wrap") as w:
                    ...
                    yield TextArea("", language="lua", show_line_numbers=True, soft_wrap=False, read_only=True, id="preview", show_cursor=True)
    ...
    def on_mount(self) -> None:
        self.apply_dusky_theme(); self.populate(); self.update_preview()
    ...
    def apply_dusky_theme(self) -> None:
        # Register a real Theme so Textual components have a proper `surface`
        # to fall back on. This matters for the read-only TextArea preview:
        # with a fully transparent backdrop its `rich_style` has no bgcolor,
        # and TextAreaTheme.apply_css asserts bgcolor is not None (using
        # `app.current_theme.surface` as the fallback).
        try:
            p = load_omarchy_palette()
        except Exception:
            p = {}
        self.register_theme(Theme(
            name="dusky_wrg",
            primary=p.get("accent", "#26c6da"),
            secondary=p.get("secondary", "#43474e"),
            background=p.get("bg", "#090E13"),
            surface=p.get("panel", p.get("bg", "#090E13")),
            warning=p.get("warning", "#fde047"),
            error=p.get("error", "#f87171"),
            success=p.get("success", "#4ade80"),
            variables={"foreground": p.get("fg", "#e1e2e9")},
        ))
        self.theme = "dusky_wrg"
    ...
    def populate(self) -> None:
        ...
        lv.clear()  # AwaitRemove: old items unmount async; we must not reuse their ids
        self._populate_seq += 1
        seq = self._populate_seq
        for i, rule in enumerate(self._filtered):
            ...
            lv.append(ListItem(Label(label), id=f"w-{seq}-{i}"))
    ...
    def update_preview(self) -> None:
        ...
        if self._filtered:
            ...
            self.query_one("#status-l", Label).update(f"{cur.app_class} :: {cur.title} • {cur.monitor.name} {cur.client.w}×{cur.client.h} • {self._status}")
    ...
    def action_refresh(self) -> None:
        try:
            ...
            err = str(e)
            self.call_from_thread(lambda err=err: self.notify(f"Refresh failed: {err}", severity="error", timeout=3))
            return
        try:
            def _apply():
                self._base = new
                self._filtered = new.copy()
                self._edits.clear()
                self._undo.clear()
                self.selected_idx = 0
                self.populate()
                self.update_preview()
                self.notify(f"Refreshed • {len(new)} windows", timeout=2)
            self.call_from_thread(_apply)
        except Exception as e:
            err = str(e)
            self.call_from_thread(lambda err=err: self.notify(f"Refresh UI error: {err}", severity="error", timeout=3))

    def on_worker_failure(self, event) -> None:
        try:
            self.notify(f"Worker crashed: {event.error}", severity="error", timeout=4)
        except Exception:
            pass
...
def main() -> None:
    # NOTE: Do NOT set TEXTUAL_NO_COLOR — we want the TUI to follow the active
    # Omarchy/thpm theme (read live from colors.toml by build_dusky_css()).
    # Enable transparent background so the TUI blends with the terminal (pacsea-style).
    os.environ.setdefault("TEXTUAL_ALLOW_TRANSPARENT", "1")
    _bootstrap_arch_deps()
    _check_hypr_version()
    ...
    # (Re)build CSS from the CURRENT active theme so colours always match the
    # system theme at launch — never falls back to Textual's default palette.
    DuskyApp.CSS = build_dusky_css(load_omarchy_palette())
    DuskyApp(rules).run()
