...
hl.window_rule({
    ...
    match = {
        class = "^(firefox)$",
        title = "^(About Mozilla Firefox|Giới thiệu về Mozilla Firefox)$"
    },
    float = true
})
...
hl.window_rule({
    ...
    keep_aspect_ratio = false  -- Locks the window frame to the video's aspect ratio
})
...
-- -----------------------------------------------------
-- qBittorrent v5.2.3 • DP-2 949×521 @ 860,447 • ws:2
hl.window_rule({
    match = {
...
-- 2. Browsers (YouTube & Streaming Media Playback)
-- Tu dong ngan khoa man hinh khi dang mo cac trang phat media / video
hl.window_rule({
    name = "idle_browser_media",
    match = {
        class = "^(firefox.*|chromium.*|google-chrome.*|brave.*|zen.*|librewolf.*)$",
        title = ".*(YouTube|Twitch|Netflix|Bilibili|Spotify|SoundCloud|Prime Video|Disney\\+|Apple TV|Jellyfin|Plex).*"
    },
    idle_inhibit = "always"
})
...
--- Strata File Manager (Blur & Opacity) ---
hl.window_rule({
    name = "strata-opacity",
    match = { class = "^(io\\.github\\.lgse\\.Strata.*)$" },
    opacity = "0.95 0.85"
})
--- Strata File Chooser Portal ---
hl.window_rule({
    name = "strata-portal-fix",
    match = { class = "^(io\\.github\\.lgse\\.Strata\\.FileChooser)$" },
    float = true,
    center = true,
    size = { 960, 640 },
    stay_focused = true,
...
-- Picture-in-Picture (Firefox) — use o.window (not hl.window_rule) so
-- geometry/pin actually enforce on the Wayland PiP window.
o.window({ class = "^(firefox)$", title = "^(Hình trong hình)$" }, {
    tag = "-default-opacity",
    float = true,
    pin = true,
    size = { 1306, 812 }, -- absolute px
    center = true,
    border_size = 0,
    opacity = "1 1",
 --   move = {754, 346},               -- local coords relative to monitor
    no_initial_focus = true,
    focus_on_activate = true,
    no_dim = true,
    opaque = true,
})
-- 60+ Phiên Bản Spiderman Đa Vũ Trụ trong 43 Phút - YouTube — Mozilla Firefox • DP-2 2542×715 @ 9,48 • ws:1
-- -----------------------------------------------------
-- 60+ Phiên Bản Spiderman Đa Vũ Trụ trong 43 Phút - YouTube — Mozilla Firefox — Generated from live window
-- DP-2 2542×715 @ 9,48 • scale 1.0 • transform 0
-- -----------------------------------------------------



-- Animation picker (kitty -e hypr_anim.sh --menu) — always float & centered
-- Note: window rules don't reliably catch exec-spawned windows, so the script
-- also self-floats via hyprctl. This rule covers hand-opened picker windows.
hl.window_rule({
    name = "anim-picker-float",
    match = {
        class = "^(hypr-anim-picker)$",
        initial_class = "^(hypr-anim-picker)$",
    },
    float = true,
    center = true,
    no_dim = true,
...
-- Omarchy shell bar (bap.floating.bar). Islands render semi-transparent
-- (alpha 0.6, or 0.35 in transparent mode) and follow the theme's bar color,
-- so the desktop behind them gets blurred instead of showing through sharp.
-- DISABLED: bar blur is left to the theme + bap.floating.bar plugin to decide
-- (avoids hardcoding effects that conflict with theme intent).
-- hl.layer_rule({
--     name = "omarchy_bar_blur",
