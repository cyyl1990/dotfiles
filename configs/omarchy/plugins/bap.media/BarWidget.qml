...
Ui.Panel {
  ...
  Service { id: localSpectrumService; shell: root.bar ? root.bar.shell : null; enableIpc: false }
  ...
  property var spectrumService: localSpectrumService
  readonly property var activePlayer: mediaService ? mediaService.activePlayer : findFallbackPlayer()
  ...
  readonly property bool playing: active && (mediaService ? activePlayer.isPlaying === true : activePlayer.playbackStatus === Mpris.Playing)
  ...
  // The persisted default comes from shell.json; currentStyle is the live
  // value and is toggled at runtime by middle-click (default <-> full).
  readonly property string mediaStyle: {
    var s = String(setting("mediaStyle", "spectrum")).toLowerCase()
    return (s === "full" || s === "spectrum") ? "full" : "default"
  }
  property string currentStyle: mediaStyle
  readonly property bool defaultMode: currentStyle === "default"
  readonly property bool fullMode: currentStyle === "full"
  ...
  // Toggle between the default (transport + marquee) and full (spectrum muse)
  // presentations. Does not persist to shell.json — resets to the saved
  // default on next shell reload.
  function toggleStyle() {
    root.currentStyle = root.fullMode ? "default" : "full"
  }
  ...
  property var parsedBorderColors: []

  FileView {
    id: activeBorderFile
    path: Quickshell.env("HOME") + "/.local/state/omarchy/current/theme/hyprland.conf"
    watchChanges: true
    printErrors: false
    onLoaded: root.loadActiveBorder(text())
    onFileChanged: reload()
    onLoadFailed: root.parsedBorderColors = []
  }

  Connections {
    target: Commons.Color
    function onAccentChanged() { activeBorderFile.reload() }
    function onForegroundChanged() { activeBorderFile.reload() }
  }

  function loadActiveBorder(raw) {
    if (!raw) { root.parsedBorderColors = []; return }
    var lines = String(raw).split("\n")
    var targetLines = []
    for (var i = 0; i < lines.length; i++) {
      var l = lines[i]
      if (l.indexOf("col.active_border") >= 0 || l.indexOf("activeBorderColor") >= 0 || l.indexOf("activeBorderGradient") >= 0) {
        targetLines.push(l)
      }
    }
    var colors = []
    var regex = /rgba?\(\s*([0-9a-fA-F]{6,8})\s*\)|#([0-9a-fA-F]{6})/g
    for (var j = 0; j < targetLines.length; j++) {
      var m
      while ((m = regex.exec(targetLines[j])) !== null) {
        var hex = m[1] || m[2]
        if (hex.length >= 6) {
          colors.push("#" + hex.substring(0, 6))
        }
      }
      if (colors.length > 0) break
    }
    root.parsedBorderColors = colors
  }

  readonly property var activeBorderColors: {
    if (parsedBorderColors && parsedBorderColors.length >= 2) {
      return parsedBorderColors
    }
    if (parsedBorderColors && parsedBorderColors.length === 1) {
      return [parsedBorderColors[0], Commons.Color.accent]
    }
    return [Commons.Color.accent, Commons.Color.urgent, Commons.Color.muted]
  }
  ...
  Item {
    ...
    implicitHeight: root.bar && root.bar.vertical
      ? (root.active ? verticalColumn.implicitHeight + 2 * root.tokens.pillPaddingX : idleIcon.implicitHeight + 2 * root.tokens.pillPaddingX)
    ...
    Ui.WidgetButton {
      ...
      onPressed: function(button) {
        ...
        else if (button === Qt.MiddleButton) root.toggleStyle()
    ...
    MediaMuse {
      ...
      playing: root.playing
      tint: root.widgetInk
      themeColors: root.activeBorderColors
  ...
  // Dead-state hint (overlay) — hiện khi spectrumService.state === "failed"
  // và widget đang ở museMode. Không chạm vào Service.qml (upstream).
  // Click overlay gọi syncSpectrum() để thử hồi sinh worker.
  Item {
    id: deadStateOverlay
    visible: root.museMode && root.spectrumState === "failed"
    anchors.fill: parent
    z: 10
    Rectangle {
      id: deadPulse
      ...
      color: "transparent"
      border.width: 1
      border.color: Qt.rgba(root.widgetInk.r, root.widgetInk.g,
        root.widgetInk.b, 0.55)
      SequentialAnimation on opacity {
        loops: Animation.Infinite
        NumberAnimation { from: 0.18; to: 0.62; duration: 1200; easing.type: Easing.InOutSine }
        NumberAnimation { from: 0.62; to: 0.18; duration: 1200; easing.type: Easing.InOutSine }
      }
    }
    IconText {
      anchors.centerIn: parent
    ...
    MouseArea {
      ...
      acceptedButtons: Qt.LeftButton
      onClicked: {
        if (root.spectrumService
            && typeof root.spectrumService.syncSpectrum === "function") {
          root.spectrumService.syncSpectrum()
        }
      }
    }
  }
