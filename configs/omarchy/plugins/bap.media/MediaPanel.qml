...
ShibumiPanel {
  ...
  readonly property bool hasMedia: mediaService
    ? (mediaService.hasMedia !== undefined
        ...
        : (player !== null && (player.trackTitle || player.trackArtist || player.isPlaying === true)))
    : false
  readonly property bool active: mediaService
    ? hasMedia && player !== null : false
  ...
  Ui.PanelKeyCatcher {
    ...
    Column {
      ...
      Item {
        ...
        // Chẩn đoán nhanh: chỉ hiện khi spectrum được bật và worker KHÔNG chạy
        // (inactive/probing/unavailable/failed) — lúc mọi thứ bình thường sẽ ẩn.
        Text {
          anchors.horizontalCenter: parent.horizontalCenter
          anchors.bottom: parent.bottom
          visible: panel.spectrumEnabled && panel.active
            && panel.spectrumState !== "running"
          text: "spectrum: " + panel.spectrumState
            + (panel.spectrumWorkerRunning ? "" : " · worker off")
          color: panel.bar ? Qt.rgba(panel.bar.foreground.r,
            panel.bar.foreground.g, panel.bar.foreground.b, 0.58)
            : Commons.Color.foreground
          font.family: panel.bar ? panel.bar.fontFamily : Commons.Style.font.family
