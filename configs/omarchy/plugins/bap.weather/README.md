...
This is the bap-plugins-omarchy fork of `io.github.daniellopez12.just-right-weather`.
The plugin lives at `~/Github/bap-plugins-omarchy/bap.weather/` and is linked into
`~/.config/omarchy/plugins/bap.weather` by `bap-suite link`.

```sh
bap-suite link bap.weather     # ensure the symlink is in place
bap-suite enable bap.weather   # add to shell.json plugins[]
```

The permanent plugin ID is `bap.weather`.
...
```sh
omarchy bar move bap.weather --section center
```
...
```sh
omarchy-shell shell summon bap.weather '{}'
omarchy-shell shell hide bap.weather
```
...
```sh
bap-suite disable bap.weather   # removes from shell.json plugins[]
bap-suite unlink bap.weather    # drops the symlink (optional)
```
