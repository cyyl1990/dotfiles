...
@/home/bap/.config/opencode/gsd-core/references/ui-brand.md
@/home/bap/.config/opencode/gsd-core/references/revision-loop.md
@/home/bap/.config/opencode/gsd-core/references/gate-prompts.md
...
- Planner is instructed to produce `SKELETON.md` in the phase directory alongside `PLAN.md`. The template lives at `/home/bap/.config/opencode/gsd-core/references/skeleton-template.md` — the planner reads it when producing SKELETON.md (lazy; not loaded on non-skeleton runs).
...
**If found:**
1. Read template: `/home/bap/.config/opencode/gsd-core/templates/VALIDATION.md`
...
**Read and run** the gate + edge probe in `/home/bap/.config/opencode/gsd-core/references/specless-probe-fallback.md`
...
**MVP_MODE:** ${MVP_MODE} (when true, follow vertical-slice rules from `/home/bap/.config/opencode/gsd-core/references/planner-mvp-mode.md`; when false, ignore MVP guidance entirely.)
**WALKING_SKELETON:** ${WALKING_SKELETON} (when true, the first deliverable must be a Walking Skeleton — Read the template at `/home/bap/.config/opencode/gsd-core/references/skeleton-template.md` and produce SKELETON.md alongside PLAN.md.)
**Granularity:** {granularity}

${MVP_MODE === 'true' ? `
<mvp_mode_active>
**MVP Mode is ENABLED.** Read `/home/bap/.config/opencode/gsd-core/references/planner-mvp-mode.md` now and follow its vertical-slice planning rules. Each plan must deliver a complete vertical slice — thin end-to-end functionality rather than horizontal layers.
</mvp_mode_active>
` : ''}
...
else-branch below, per §A/§B/§C of `/home/bap/.config/opencode/gsd-core/references/specless-probe-fallback.md`
...
When the planner returns `## PHASE SPLIT RECOMMENDED`, the phase's source items exceed the context budget for full-fidelity implementation. Extract the planner's proposed sub-phase groupings and present the user three options via question: Split into sub-phases (use `/gsd-phase --insert`, then replan each), Proceed anyway (return to planner accepting degraded quality), or Prioritize (question multiSelect to choose now vs. later, create CONTEXT.md per sub-phase). Full banner text: `gsd-core/workflows/plan-phase/detail/elaboration.md` § 9b.
